...
def train_loop(config: dict):
    ...
    w_loss, w_skel = config[HyperParameters.WEIGHTS.value]
    losses_weights = LossesWeights(w_loss, w_skel)
    ...
    n_layers = config[HyperParameters.LAYERS.value]
    n_heads = config[HyperParameters.HEADS.value]
    
    #Init the transformer model
    model = HwTransformer(use_lstm=USE_LSTM, lstm_module=lstm_model,
                            use_prediction_token=USE_PRED_TOKEN, make_positional_encodings_trainable=IS_POSITION_LEARNABLE,
    ...
    # Training loop
    optimizer = Adam(model.parameters(), lr=config[HyperParameters.LR.value])
