...
\begin{tcolorbox}[colframe=black, colback=white, boxrule=1pt, arc=4mm, title=Abstract]
\noindent
This thesis explores the reconstruction of online handwriting signals (e.g., pen trajectories) 
from offline images using transformer-based architectures.\par 
The study addresses the challenge of modeling handwriting kinematics, focusing on trajectories and velocity, 
through a transformer architecture.\par 
...
Future directions include extending this approach to historical documents, improving noise robustness, changing absolute coordinates to 
relative speed vectors, and refining datasets for broader handwriting styles.
\end{tcolorbox}
...
\textit{With the assistance of Ray, PHD candidate, Department of Informatics, University of Fribourg}
...
The results of the re-run of this combination are shown in the side-by-side graphs on the figure \ref{fig:rerun_side_by_side}. It is interesting to observe in the detailed train-test losses per batch in the \ref{fig:rerun_detailed_train_test} that the re-run suffered the same fate of exploding loss during the last epoch. This explosion altered the final average train loss to reach the scale of hundreds, similarly as during the initial grid-search. Although the model is capable of getting back on track after a sufficient number of batches, the test loss is still negatively impacted as shown in the \ref{fig:rerun_train_test}. This final test loss is added to the result table as an additional point of information, but the fact that two identical problems happened on the same combinations can be indicative of a bad mix. \\
...
The comparison will be structured around the quality of the generated signals during inference on the same validation dataset containing \(2000\) signals, of which half are considered "small" and half are "large", with a respective length \(15 <= length <= 50\) and \(50 <= length\). The first point of comparison is the general length of the generated signal the models tend to have a hard time stopping. The second point of comparison are the global evaluation metrics - DTW, Euclidean distances, and image differences - to compare the three models.
...
The distances are only one side of the coin as they inform us only of the capacity of the model to stick to the original signal, but different explanations could result in the slight loss of performance for augmented models. For example, different 
prediction speed will induce a latent variation of the distances without impacting the shape of the image. We also observe that the augmentation of the point-to-point distance does not correlate with an augmentation of the DTW distance for the mixed model.
...
The conclusion of the experiment is that the addition of the augmentation itself is beneficial, especially for the predicted image skeletons sticking better to the originals,
although we observe better results when the model is able to train with both the "perfect" (unaugmented) and the noised signals. 
Although this double the size of the dataset, this produces better results and the mix of both is consistently better than one or the other.
...
To give some perspective over the metrics used for comparison during the experiments, 
the above prediction scored \([52\%,48\%,50\%]\) on image matching and \([97, 149]\) on DTW and Point-to-Point Euclidean distance 
hile the below prediction scored \([44\%,40\%,42\%]\) matching and \([167, 435]\) DTW and distance. These results are located between the average and the third quartile 
...
to offsets in the image. It is hard to find good evaluators and more exploration around this problematic would be helpful. \\ 
...
is still an issue to keep on the radar for enhancements. \\ 
...
\section{LSTM results} \label{section:appendix_lstm}
